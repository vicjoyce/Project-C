...
import com.brianku.qbchat.adapters.OpponentsCallAdapter
...
import kotlinx.android.synthetic.main.fragment_video_conversation.*
...
class VideoConversationFragment : BaseToolBarFragment(), QBRTCSessionStateCallback<QBRTCSession>,
    ...
    private var opponents =  ArrayList<QBUser>()
    ...
    override fun onStart() {
        ...
        if (currentSession!!.state != BaseSession.QBRTCSessionState.QB_RTC_SESSION_CONNECTED) {
            if (isIncomingCall) {
                Toast.makeText(activity,"onStart-acceptCall",Toast.LENGTH_LONG).show()
...
//              currentSession!!.startCall(null)
        ...
        Toast.makeText(activity,"onStart-startCall",Toast.LENGTH_LONG).show()
...
//        initSessionListeners()
    ...
    private fun initArguments() {
        arguments!!.let {
            isIncomingCall = it.getBoolean("conversation_reason")
            val obj = it.get("qbUser")
            if (obj is ArrayList<*>) {
                opponents = obj.filterIsInstance<QBUser>() as ArrayList<QBUser>
            }else{
                Log.d(TAG,"$obj")
                opponent = obj as QBUser
